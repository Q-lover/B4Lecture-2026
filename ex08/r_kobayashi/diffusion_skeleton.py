...
class DiffusionModel(nn.Module):
    ...
    def q_sample(
        ...
        # ブロードキャストのために alpha_prod_t の形状を (B, 1, 1, 1) に変換
        alpha_prod_t = self.alpha_prod[t].view(-1, 1, 1, 1)  # (B, 1, 1, 1)
    ...
    def p_sample(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        eps_theta = self.model(x, t).sample  # 推定ノイズ ε_θ(x_t, t)

        alpha_t = self.alpha[t].view(-1, 1, 1, 1)  # (B, 1, 1, 1)
        alpha_bar_t = self.alpha_prod[t].view(-1, 1, 1, 1)  # (B, 1, 1, 1)
    ...
    def training_step(self, images: torch.Tensor) -> torch.Tensor:
        ...
        noise_pred = self(x_t, t)  # 推定ノイズ ε_θ(x_t, t)
